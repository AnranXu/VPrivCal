...
export interface PreExpertSimulationReport {
  ...
  ruleChecks: {
    ...
    q8FilteredDecisionCount: number;
...
const profiles: readonly SimulatedProfileDefinition[] = [
  ...
  },
  {
    ...
    rationale: 'Requests avoidance for every category and cross-cutting condition.',
    q10Values: q10Values(5, 5, 5, 5, 5),
    defaultProbeAction: 4,
    awarenessStatus: 3,
    expectedPreferenceRanks: {
      [candidateIds.face]: 4,
      [candidateIds.basketball]: 4,
      [candidateIds.paySlip]: 4,
      [candidateIds.altar]: 4,
      [candidateIds.lab]: 4,
      [candidateIds.name]: 4,
      [candidateIds.pilotChildren]: 4,
      [candidateIds.pilotOffering]: 4,
      [candidateIds.pilotName]: 4,
      [candidateIds.pilotBasketball]: 4,
    },
  },
  {
    profileId: 'sim_context_dependent',
    label: 'Context dependent',
    rationale: 'Uses different actions for private, public, and semi-public observations.',
    q10Values: q10Values(3, 2, 4, 2, 2),
    defaultProbeAction: 2,
    probeActionOverrides: {
      'scene_private_family_party|biometric_data': 1,
      'scene_private_family_party|children_images': 2,
      'scene_private_family_party|pii': 4,
      'scene_private_family_party|legal_sensitivity_information': 2,
      'scene_private_family_party|personal_life': 1,
      'scene_public_cafe|biometric_data': 2,
      'scene_public_cafe|children_images': 2,
      'scene_public_cafe|pii': 3,
      'scene_public_cafe|background_individuals': 3,
      'scene_semipublic_hospital|biometric_data': 2,
      'scene_semipublic_hospital|pii': 3,
    },
    awarenessStatus: 2,
    expectedPreferenceRanks: {
      [candidateIds.face]: 1,
      [candidateIds.basketball]: 3,
      [candidateIds.paySlip]: 4,
      ...
      [candidateIds.pilotName]: 4,
      [candidateIds.pilotBasketball]: 3,
    },
  },
  {
    profileId: 'sim_uncertainty_sensitive',
    label: 'Uncertainty sensitive',
    rationale: 'Usually handles silently but avoids use when a cue is uncertain.',
    q10Values: q10Values(2, 3, 5, 5, 1),
    defaultProbeAction: 1,
    awarenessStatus: 3,
    expectedPreferenceRanks: {
      [candidateIds.face]: 1,
      [candidateIds.basketball]: 1,
      [candidateIds.paySlip]: 1,
      [candidateIds.altar]: 2,
      [candidateIds.lab]: 1,
      [candidateIds.name]: 4,
      [candidateIds.pilotChildren]: 1,
      [candidateIds.pilotOffering]: 4,
      [candidateIds.pilotName]: 1,
      [candidateIds.pilotBasketball]: 1,
    },
  },
  {
    profileId: 'sim_reminder_averse',
    label: 'Reminder averse',
    rationale: 'Requests reminders in principle but Q8 retains them only at risk tier five.',
    q10Values: q10Values(3, 1, 1, 1, 1),
    defaultProbeAction: 2,
    awarenessStatus: 1,
    expectedPreferenceRanks: {
      [candidateIds.face]: 2,
      ...
      [candidateIds.altar]: 1,
      [candidateIds.lab]: 1,
      [candidateIds.name]: 1,
      [candidateIds.pilotChildren]: 2,
      [candidateIds.pilotOffering]: 1,
...
export function runPreExpertSimulation(
  ...
  return {
    ...
    ruleChecks: {
      ...
      q8FilteredDecisionCount: everyDecision.filter(({ reasons }) =>
        reasons.includes('notification_filtered_by_Q8'),
      ).length,
